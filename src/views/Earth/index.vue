    ...
    <div class="earth-silder">
      <div class="earth-list">
        ...
        >
          ...
          >
            <div
              class="earth-item_btn"
              @click="delScene($event, item)"
            >
              <img
    ...
    >
      ...
      >
        ...
        <el-form-item>
          ...
          <el-button
            size="mini"
            @click="isShowAddSceneDialog = false"
          >取 消</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
...
export default {
  ...
  methods: {
    getCapture() {
      // html2canvas(document.querySelector('#earthContainer')).then(function (
      //   canvas
      // ) {
      //   document.body.appendChild(canvas);
      // });
      // console.log(this._earth._earth.capture);
      this._earth._earth
        .capture(1920, 1080)
        .then((img) => {
          this.scene_snapshot = img;
          let index = this.sceneList.findIndex(
            (item) => item.rowid === this.currectId
          );
          this.sceneList[index].scene_snapshot = this.scene_snapshot;
          sessionStorage.setItem('scene', JSON.stringify(this.sceneList));
    ...
    saveData() {
      let data = {
        sceneTree: {
          ...this._earth._earth.sceneTree
        }
      };
      console.log(data, 'saveData================');
      let scene = sessionStorage.getItem('scene') || '[]';
      this.sceneList = JSON.parse(scene);
      let rowid = sessionStorage.getItem('sceneid');
      let index = this.sceneList.findIndex((item) => item.rowid === rowid);
      this.sceneList[index].scene_json = JSON.stringify(data);
      sessionStorage.setItem('scene', JSON.stringify(this.sceneList));
    ...
    async laodData() {
      ...
      this.sceneList &&
        ...
        this.changeScene(this.sceneList[0]);
      return;
      const {
        data: { rows }
      } = await getFilterRows({
...
.xbsj-nav
  > div
  .xbsj-template:nth-child(1)
  .xbsj-list
  .xbsj-list-item:nth-child(1) {
  display: none;
}
.earth-content {
  position: relative;
