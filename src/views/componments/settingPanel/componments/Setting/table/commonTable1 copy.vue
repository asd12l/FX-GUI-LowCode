      ...
      <el-collapse>
        <el-collapse-item title="表格数据">
          ...
          <div class="button-box">
            <el-button type="danger" size="mini" @click="clearShowTxt"
              >清除文本</el-button
            >
            <el-button
              type="primary"
        ...
        <el-collapse-item title="搜索项">
          ...
          <el-collapse-item title="选项排序：">
            <div
              ...
              :key="index"
            >
              <el-form-item :label="val.data.label">
                <el-input
                  v-model="val.sort"
                  size="mini"
                  placeholder="请输入排序"
        ...
        <el-collapse-item title="卡片式数据展示内容">
          ...
          <div class="button-box">
            <el-button type="danger" size="mini" @click="clearShowCardTxt"
              >清除文本</el-button
            >
            <el-button
              type="primary"
          ...
          <el-collapse-item title="按钮内容">
            ...
            <div class="button-box">
              <el-button type="danger" size="mini" @click="clearShowButtonTxt"
                >清除文本</el-button
              >
              <el-button
                type="primary"
...
export default {
  ...
  methods: {
    ...
    getSelectData() {
      let d = [...this.config.otherData.selectTypeData];
      let t = this.config.otherData.tableHead.filter(
        (item) => item.seletedType
      );
      let k = [];
      let keys = [];
      if (d.length > 0) {
        d.forEach((item) => {
          this.selectData[item.data.value] = item;
      ...
      t.forEach((item, i) => {
        ...
        let seat = keys.indexOf(item.value);
        if (item.seletedType !== "search") {
          k.push(item.value);
          this.selectData[item.value] = {
            ...
            width: seat > 0 ? d[seat].width : "",
            height: seat > 0 ? d[seat].height : 32,
            marginLeft: seat > 0 ? d[seat].marginLeft : "",
            marginTop: seat > 0 ? d[seat].marginTop : "",
            sort: seat > 0 ? d[seat].sort : i + 1,
          };
        } else {
          if (k.indexOf("keyWord") === -1) {
            k.push("keyWord");
          }
          if (!this.selectData.keyWord) {
      ...
      let key = Object.keys(this.selectData);
      if (key.length > k.length) {
        key.forEach((item) => {
          console.log(item, k, k.indexOf(item), "===item");
          if (k.indexOf(item) == -1) {
            delete this.selectData[item];
          }
        });
      }
      let s = Object.values(this.selectData);
      this.$emit("changeValue", "otherData", "selectTypeData", s);
    },
    changeSort() {
      let d = [...this.config.otherData.selectTypeData].sort(this.down("sort"));
      this.$emit("changeValue", "otherData", "selectTypeData", d);
      let s = {};
      d.forEach((item) => {
        s[item.data.value] = this.selectData[item.data.value];
      });
      this.selectData = s;
    },
    down(type) {
    ...
    addSelect() {
      let s = [...this.config.otherData.selectTypeData];
      s.push({
        data: {},
        width: "",
        height: 32,
