<template>
  <el-scrollbar class="right-setting scrollbar-wrapper">
    ...
    <poiEarthDialog 
      @onSureCb="data => $emit('changeSize', 'styleObj', data)" 
      :styleObj="styleObj"
      v-if="isShowEarthDialog" 
      :dialogVisible="isShowEarthDialog"
      :type="'billBoard'"
      @changeIsShowEarthDialog="status => isShowEarthDialog = status"
    ></poiEarthDialog>
  </el-scrollbar>
</template>
...
export default {
  ...
  computed: {
    styleObj() {
      console.log("this.config.styleObj:::::::::::", this.config.styleObj)
      return this.config.styleObj
    },
    styleStr() {
      return JSON.stringify(this.config.styleObj)
    }
  },
  methods: {
