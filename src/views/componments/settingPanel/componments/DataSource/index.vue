    ...
    >
      ...
      <div v-show="config.sourceType === 'reportId'">
        ...
        >
          <el-input
            ...
            @change="(val) => $emit('changeSize', 'reportName', val)"
            size="mini"
            disabled
          ></el-input>
        </el-form-item>
      ...
      <div v-show="config.sourceType === 'api'">
        ...
        <el-form-item label="请求方法">
          <el-select
            ...
            @change="(val) => $emit('changeSize', 'requestType', val)"
            placeholder="请选择请求方法"
          >
            <el-option
              label="GET"
        ...
        <el-form-item
          v-if="config.requestType === 'POST'"
          label="请求参数"
        >
          <vue-json-editor
            class="json-editor"
            v-model="config.requestParams"
            :showBtns="false"
            :mode="'code'"
            lang="zh"
            :expandedOnStart="true"
            @json-change="(val) => $emit('changeSize', 'requestParams', val)"
          />
        </el-form-item>
        <!-- <el-input
          v-model="config.api"
        ...
        <el-form-item>
          <template slot="label">
            ...
            <el-switch
              v-model="config.isShowDataFilter"
              @change="(val) => $emit('changeSize', 'isShowDataFilter', val)"
            ></el-switch>
          </template>

          <CodeEdit
            v-show="config.isShowDataFilter"
            v-model="config.datafilterFunc"
      ...
      >
        ...
        <el-button
          type="primary"
          size="mini"
          @click="isShowWorkSheetDialog = true"
        >选择工作表</el-button>
      </div>
      ...
      >
        <el-form-item
          label="是否开启字段映射"
          class="isOpenFieldMap"
        >
          <el-switch
            style="margin-left: 10px;"
            v-model="config.isOpenFieldMap"
            @change="(val) => $emit('changeSize', 'isOpenFieldMap', val)"
          ></el-switch>
        </el-form-item>
      </div>

      <el-button
        v-show="config.isOpenFieldMap"
        type="primary"
        size="mini"
        @click="isShowFieldMapDialog = true"
      >字段映射</el-button>
    </el-form>
    <reportIdDialog
      @toClose="isShowReportIdDialog = false"
      @toSave="toSave"
      @changeButtonData="changeButtonData"
      :dialogFormVisible="isShowReportIdDialog"
      :type="this.config.component"
    >
    </reportIdDialog>
    <workSheetDialog
      @toClose="isShowWorkSheetDialog = false"
      @toSave="toSaveWorkSheetDialog"
      :dialogFormVisible="isShowWorkSheetDialog"
      :type="this.config.component"
    >
    </workSheetDialog>
    <fieldMapDialog
      @toClose="isShowFieldMapDialog = false"
      @toSave="toSaveFieldMap"
      @changeValue="
        (param1, param2, val) => $emit('changeValue', param1, param2, val)
      "
      :config="config"
      :data="fieldMapData"
      v-if="isShowFieldMapDialog"
      :isShow="isShowFieldMapDialog"
    ></fieldMapDialog>
  </el-scrollbar>
</template>
<script>
...
import reportIdDialog from './reportIdDialog';
...
export default {
  ...
  methods: {
    selectSourceType(val) {
      if (val === '工作表' || val === 'reportId') {
        this.$message.warning('暂未开放!');
      } else {
        this.$emit('changeSize', 'sourceType', val);
      }
    },
    changeData(filed, data) {
      if (this.config.component === 'commonTable1') {
        this.$emit('changeValue', 'otherData', filed, data);
      } else {
        this.$emit('changeSize', filed, data);
      }
    },
    // 字段映射
    toSaveFieldMap(data) {
      // 根据组建的映射字段  进行数据处理
      const filterData = fieldMap[this.config.component]
        ? fieldMap[this.config.component](data, this.config, this.changeData)
        : [];
      console.log(data, filterData, '====filterData');
      this.$emit('changeSize', 'fieldMap', data);
      this.$emit('changeSize', 'data', filterData);
    ...
    // 加载接口数据
    async toLoadData() {
      console.log(this.config, window.baseURL, '"""""""""""""""""confiig');
      try {
        const api =
          this.config.api.indexOf('http') >= 0
            ? this.config.api
            : window.baseURL + this.config.api;
    ...
    async saveWorkSheetId() {
      const { data } = await getFilterRows({
        ...
        pageSize:
          (this.config.requestParams && this.config.requestParams.pageSize) ||
          100,
        pageIndex: 1
      });
      this.fieldMapData = data.rows;
      if (this.config.component === 'commonTable1') {
        this.$emit('changeSize', 'data', {
          tableData: data.rows,
          total: data.total
        });
      } else if (this.config.component === 'scrollTable') {
        this.$emit('changeSize', 'data', data.rows);
      } else {
        this.$emit('changeSize', 'data', data);
      }
    },
    async saveReportId() {
      console.log('数据更新saveReportId');
