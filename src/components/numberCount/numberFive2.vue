              ...
              <template>
                <div
                  ...
                  :style="{
                    ...
                    borderColor:
                      active == index
                        ? config.contain.borderColor
                        : 'transparent',
                    background: config.contain.show
                      ? `url(${
                          config.contain.background
                        }) 100% 100%/100% 100% no-repeat `
                      : config.contain.backgroundColor,
                  }"
                >
                  <div
                  ...
                  >
                    <span
                      ...
                      >{{ item.numleft }}</span
                    ><span v-if="config.contain.contain">{{
                      config.contain.contain
                    }}</span><span
                      class="camera-line-all-total"
                      :style="{
    ...
    "config.data": {
      handler(nVal, oVal) {
        ...
        if (nVal.length && d.contain.arrangeNum) {
          if (
            d.txtRowImgs.length !== nVal.length &&
            nVal.length > d.txtRowImgs.length
          ) {
            let num = nVal.length - d.txtRowImgs.length;
            for (let i = 0; i < num; i++) {
              d.txtRowImgs.push({
            ...
            d.txtRowImgs = d.txtRowImgs.slice(0, nVal.length);
          }
