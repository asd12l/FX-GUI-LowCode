...
export default {
  ...
  computed: {
    ...
    defaultOption() {
      return {
        step: 0.3, // 数值越大速度滚动越快
        limitMoveNum: Math.ceil(this.config.width / this.config.box.width), // 开始无缝滚动的数据量 this.dataList.length
  ...
  methods: {
    test(arr,num){
      return arr.reduce((t, l, i) => {
   
        if( !t.length || t.at(-1).length >= Math.ceil(arr.length / num) || arr.length - i === num - t.length) {
            t.push([]);
        }
        t.at(-1).push(l)
        return t;
